...
class Net_backbone3(nn.Module):
    ...
    def _update_stem(self, backbone):
        ...
        # ---- ConvNeXt v1 (small/base など) ----
        if backbone.startswith("convnext_small") or backbone.startswith("convnext_base"):
            stem_conv = self.backbone.stem_0
            stem_conv.stride  = (4, 1)
            stem_conv.padding = (0, 2)
            # ↓縦方向ダウンサンプル用の new_conv を追加（既存ロジック）

        # ---- ConvNeXt **v2** ----
        elif backbone.startswith("convnextv2"):
            # v2 は stem が nn.Sequential([...]) なので 0 番目の Conv を取得
            ...
            with torch.no_grad():
                ...
                new_conv = nn.Conv2d(
                    w.shape[0], w.shape[0],
                    kernel_size=(4, 4), stride=(4, 1), padding=(0, 1)
                )
                new_conv.weight.copy_(w.repeat(1, (128 // w.shape[1]) + 1, 1, 1)
                                    [:, :new_conv.weight.shape[1], :, :])
                new_conv.bias.copy_(stem_conv.bias)

            # stem をラップして高さ方向だけ 4× down-sample
